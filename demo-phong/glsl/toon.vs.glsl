...
void main() {
    ...
    vec3 viewNormal = normalize(vec3(inverse(transpose(modelViewMatrix))*vec4(normal, 1.0))); // HINT: Needs to be in VCS

    fresnel = dot(normalize(viewDirection), normalize(viewNormal));
